...
def load_fn_by_name(module_name, function_name):
    full_import_path = f"{module_name}"
    fn_module = importlib.import_module(full_import_path)
    return getattr(fn_module, function_name)


DEFAULT_FNS = [
    "slambda.contrib.sentiment:sentiment",
    "slambda.contrib.entail:entail",
    "slambda.contrib.summarize:summarize",
    "slambda.contrib.motivate:motivate_me",
    "slambda.contrib.wiki_link:extract_wiki_links",
    "slambda.contrib.writing.grammar:fix_grammar",
    "slambda.contrib.writing.essay:generate_essay",
]
